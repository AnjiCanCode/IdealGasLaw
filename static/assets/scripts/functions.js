...
var NavigationInfo = new function ()
{
    ...
    if (panelIndex < 0)
    {
        // Gracefully handle pages not in the navigation
        this.currentPosition = {
            sequence: { baseUrl: "/", panelUrls: ["/"], title: "Beyond Ideal" },
            panelIndex: 0,
        };
    }
    else
...
addChapter("gasBasics", []);
addChapter("idealGasLaw", [chapters.gasBasics]);
addChapter("kineticTheory", [chapters.idealGasLaw]);
addChapter("realGas", [chapters.idealGasLaw]);
addChapter("vanderWaals", [chapters.realGas]);
addChapter("mixtures", [chapters.idealGasLaw]);
...
function updateBounds(simulation, boxWidth, boxHeight)
{
    ...
    if (simulation.renderer.bounds.width === 0) {
